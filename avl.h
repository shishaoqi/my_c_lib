...
/*
 * Function: NewAVL
 * Usage: avl = NewAVL(sizeof (nodeT), cmpFn, nodeInitFn);
 * -------------------------------------------------------
...
/*
 * Function: FindAVLNode
 * Usage: np = FindAVLNode(avl, &key);
 * -----------------------------------
 ...
 * Usage: np = InsertBSTNode(avl, &key, clientData);
 * -------------------------------------------------
 ...
 * Usage: np = DeleteBSTNode(avl, &key);
 * -------------------------------------
...
/*
 * Function: MapBST
 * Usage: MapBST(fn, avl, order, clientData);
 * ------------------------------------------
...
void MapAVL(nodeFnT fn, avlADT avl, traversalOrderT order,
            void *clientData);

/*
 * Low-level functions: BSTRoot, BSTLeftChild, BSTRightChild
 * Usage: root = BSTRoot(avl);
 *        child = BSTLeftChild(avl, np);
 *        child = BSTRightChild(avl, np);
 * ---------------------------------------------------------
